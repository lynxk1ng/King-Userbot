...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**Maaf Orang Tampan Sedunia {ALIVE_NAME} Sedang SIBUK**\n╰╼═════════╾",
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**Orang Tampan {ALIVE_NAME} Sedang SIBUK**\n**Tunggu Sampai Orang Tampan Online Kembali**\n╰╼═════════╾",
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**Orang Tampan{ALIVE_NAME} Sedang SIBUK**\n**Tunggulah Sampai Orang Tampan Online**\n╰╼═════════╾",
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**Orang Tampan {ALIVE_NAME} Sedang SIBUK**\n╰╼═════════╾",
]
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**🔥 Sibuk**\n\n╭╼══════════════╾\n**Orang Tampan {ALIVE_NAME} Sok" an Sibuk!**\
        \n╰► **Alasan :** `{string}`\n╰╼═════════╾")
    else:
        await afk_e.edit(f"**🔥 Sibuk**\n\n╭╼══════════════╾\n**Orang Tampan {ALIVE_NAME} Sok" an  Sibuk!**\n╰╼═════════╾")
    if user.last_name:
        ...
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"🔥 __**PESAN**__\n\n╭╼══════════════╾\n**{ALIVE_NAME} Orang Tampan Masih SIBUK**\n**Sejak :** {afk_since} Yang Lalu\
                            \n╰► **Alasan :** `{AFKREASON}`\n╰╼═════════╾")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"🔥 __**PESAN**__\n\n╭╼══════════════╾\n**{ALIVE_NAME} Tungguin Ampe Gak SIBUK Mohon Tunggu Ya kawand!**\n**Sejak :** {afk_since} Yang Lalu\
                        \n╰► **Alasan :** `{AFKREASON}`\n╰╼═════════╾")
                else:
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply(f"🔥 __**PESAN**__\n\n╭╼══════════════╾\n**{ALIVE_NAME} Udah Gua Bilang Masih SIBUK**\n**Sejak :** {afk_since} Yang Lalu\
                            \n╰► **Alasan :** `{AFKREASON}`\n╰╼═════════╾")
                    else:
                        await sender.reply(str(choice(AFKSTR)))
...
CMD_HELP.update(
    {
        "sibuk": "**✘ Plugin : **`sibuk`\
        \n\n  •  **Perintah :** `.sibuk`\
        \n  •  **Function : **Memberi tahu kalau Orang Tampan sedang SIBUK , dan menguubah nama belakang menjadi ⚡️𝗦𝗜𝗕𝗨𝗞⚡️\
        \n\n  •  **Notes :** __Kalo ada orang spam berlebihan ke Elu , tinggal ketik__ `.block`\
    "
    }
)
